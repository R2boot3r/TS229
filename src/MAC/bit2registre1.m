...
function registre_ouput = bit2registre1(bitPacketCRC,refLon,refLat)
%% Variables
tableau_char = '_ABCDEFGHIJKLMNOPQRSTUVWXYZ_____ _______________0123456789______';
registre_input = struct('adresse',[],'format',[],'type',[],'nom',[],'altitude',[],'timeFlag',[],'cprFlag',[],'latitude',[],'longitude',[],'trajectoire',[]);


% D�clarations des variables/indices local de positions
%indice trame ADS-B
index_pre = 7; %Attention bitPacket ne contient pas le pr�ambule il faut donc d�caler toutes les valeurs suivantes de 7
...
bitPacket = bitPacket';% information decod�
...
if error_flag == 0 % prise en compte des tram sans erreurs, aucune erreur sur les donn�
    ...
    if DF == 17
        ...
        if registre_input.type >= 1 && registre_input.type <= 4
           ...
           char1 = bin2dec(num2str(ADS(9:14))); %% � utiliser reshape
...
%compl�t� on �crase a chaque fois l'ancienne structure